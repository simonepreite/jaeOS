...
int insertBlocked(int *semAdd, struct pcb_t *p){
	...
	int trovato = FALSE; // semAdd not found: FALSE
                      // semAdd found:     TRUE
	...
	char *g = "semaforo e processo inseriti for\n";
	char *a = "sono nell'if di insertBlocked\n";
	char *c = "sono nel secondo if\n";
	char *b = "non ho fatto nulla\n";
	char *o = "dequeue semdFree\n";
	char *q = "verifico se semdFree e' vuota\n";
	char *z = "semaforo e processo inseriti clist_enqueue\n";
	char *v = "scroll aslh\n";*/
	if (p != NULL) { 
			//tprint(a);
			if(aslh.next != NULL){
		...
		if (trovato) { // semaforo trovato
			...
			if (semdFree.next != NULL) { //semdFree non vuota
				...
				//clist_foreach(scan, &aslh, s_link, tmp) tprint(v);
				return FALSE;
				}
